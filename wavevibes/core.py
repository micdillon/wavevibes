...
class AudioProcessor:
    ...
    def process_file(
        ...
        if self.algorithm_factory:
            ...
            if (
                self.algorithm.sample_rate != sample_rate
                or self.algorithm.channels != n_channels
                or self.algorithm.block_size != self.block_size
            ):
                # Warn user about parameter mismatch
                print(
                    "Warning: Algorithm initialized with different parameters than input file"
                )
                print(
                    f"  Algorithm: {self.algorithm.sample_rate}Hz, {self.algorithm.channels}ch"
                )
